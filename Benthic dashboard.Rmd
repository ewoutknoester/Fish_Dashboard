...
# ---- RAW DATA ----
## load invert xls sheet
df0.raw <- read_excel("Raw data/Benthic surveys_2024-07.xlsx", sheet = "Inverts POINT")

## Convert to long dataframe
df1.clean <- as.data.frame(panelr::long_panel(df0.raw, prefix = "_P", begin = 1, end = 600, label_location = "end", check.varying = FALSE))

## Cleanup data frame
df1.clean <- select(df1.clean, -c('id','Comments'))
...
names(df1.clean)[1] <- "Survey"

# Cleanup groups
df1.clean <- subset(df1.clean, Group != "Crabs") # Exclude crabs (not consistently surveyed)
df1.clean <- subset(df1.clean, Species != "Tridacna sp") # Exclude Tridacna: not motile invertebrate
df1.clean$Group[df1.clean$Group == "Shells"] <- "Gastropods" # Rename

# ---- META DATA ----
## Load Excel
meta <- read_excel("Raw data/Benthic surveys_2024-07.xlsx", sheet = "Metadata POINT")
meta <- select(meta, c('Survey', 'Date', 'Location', 'Transect', 'Depth', 'Observer', 'Survey.type', 'Comments'))
meta <- meta[!is.na(meta$Date),]
## Change survey names into numbers
meta$Survey <- sub(".", "", meta$Survey)
meta$Survey  = str_remove(meta$Survey, "^0+")
## Set NAs to blanks
meta$Comments <- ifelse(is.na(meta$Comments), "NA", meta$Comments)
...
meta$Transect <- tolower(meta$Transect)
transectinfo$Transect <- tolower(transectinfo$Transect)

meta <- left_join(meta, transectinfo, by = "Transect")

# Cleanup
meta$Type <- ifelse(meta$Transect == "na", "Reference", meta$Type)
meta$Site <- ifelse(meta$Transect == "na", meta$Location, 
             ifelse(is.na(meta$Site), meta$Location, meta$Site))
meta$Type <- ifelse(is.na(meta$Type), "Other", meta$Type)
colnames(meta)[which(names(meta) == "Type")] <- "Treatment"
meta$Patch.type <- ifelse(grepl("cma-l-", meta$Transect), "AR (L)",
                   ifelse(grepl("cma-xl-", meta$Transect), "AR (XL)",
                   ifelse(grepl("cma-xs-", meta$Transect), "AR (XS)",
                   ifelse(grepl("-bru-", meta$Transect), "BRU",
                   ifelse(grepl("-cage-", meta$Transect), "CAGE",       
                   ifelse(grepl("-cake-", meta$Transect), "CAKE",       
                   ifelse(grepl("-comp-", meta$Transect), "COMP",
                   ifelse(grepl("m-", meta$Transect), "MOSES (- coral)",       
                   ifelse(grepl("m+", meta$Transect), "MOSES (+ coral)",       
                   ifelse(grepl("m-c", meta$Transect), "MOSES (control)",  
                   ifelse(grepl("m-p", meta$Transect), "MOSES (pins)",       
                   ifelse(grepl("r-l-", meta$Transect), "Reference (L)",
                   ifelse(grepl("r-xl-", meta$Transect), "Reference (XL)",
                   ifelse(grepl("r-xs-", meta$Transect), "Reference (XS)", "na"))))))))))))))     
meta$Date.created <- excel_numeric_to_date(as.numeric(meta$Date.created))
meta$Year.created <- year(meta$Date.created)
meta$Year <- year(meta$Date)
meta$Survey <- as.numeric(meta$Survey)
...
meta <- dplyr::select(meta, -c("Date.created"))

# ---- MERGE ----
df1.clean <- subset(df1.clean, Survey <= max(meta$Survey))
df1.clean <- left_join(df1.clean, meta, by = "Survey")

## Re-order
df1.clean <- dplyr::select(df1.clean, "Survey", "Survey.type", "Site", "Treatment", "Patch.type", "Transect", "Year.created", "GPS",
...
df1.clean$Survey.area <- ifelse(df1.clean$Survey.type == "Circle.10" & df1.clean$Group == "Corallivorous.snails", 10,
                         ifelse(df1.clean$Survey.type == "Circle.10" & df1.clean$Group == "Urchins", 20,
                         ifelse(df1.clean$Survey.type == "Circle.10", 50,
                         ifelse(df1.clean$Group == "Corallivorous.snails", 20,
                         ifelse(df1.clean$Group == "Urchins", 40, 100)))))

df1.clean$Density_m2 <- df1.clean$Abundance / df1.clean$Survey.area
df1.clean <- dplyr::select(df1.clean, -c("Survey.type", "Abundance", "Survey.area"))

write_xlsx(df1.clean, "Inverts_All years.xlsx")
...
# Load
inverts <- as.data.frame(read_excel("Inverts_All years.xlsx"))

# Set data types
inverts$Site <- as.factor(inverts$Site)
inverts$Treatment <- as.factor(inverts$Treatment)
inverts$Patch.type <- as.factor(inverts$Patch.type)
inverts$Transect <- as.factor(inverts$Transect)
inverts$Observer <- as.factor(inverts$Observer)
inverts$Year <- as.factor(inverts$Year)
inverts$Species <- as.factor(inverts$Species)

# Select
inverts.selex <- subset(inverts, 
...
inverts.sum <- inverts.selex

# Select relevant columns
inverts.sum <- dplyr::select(inverts.sum, c("Survey", "Site", "Treatment", "Patch.type", "Transect", "Observer", "Year",
                              "Species", "Group", "Density_m2"))
# Summarize over species
library(dplyr)
inverts.sum <- inverts.sum %>%
   group_by(Group, Species) %>%
   dplyr::summarise(Density_m2 = mean(Density_m2), Surveys.count = n())
...
inverts.sum <- subset(inverts.sum, Density_m2 > 0)

## Write clean Excel                     
write_xlsx(inverts.sum, "Reference reefs_KH-CMA-PP_2017-2024_Inverts.xlsx")     
